...
{
		// // --- Debug logging block ---
		// static long long getNextSlice_iteration = 0;
		// getNextSlice_iteration++;
		// if (getNextSlice_iteration % 1000000 == 0) {
		// 	std::ofstream dbg("calculateSlice_debug.log", std::ios::app);
		// 	dbg << "getNextSlice call #" << getNextSlice_iteration
		// 		<< " | Eq=" << Eq
		// 		<< " | hinP=" << hinP
		// 		<< " | hinN=" << hinN
		// 		<< std::endl;
...
// #endif
	...
	{
		// // --- Debug logging block ---
		// static int calculateNodeClipPrecise_iteration = 0;
		// calculateNodeClipPrecise_iteration++;
		// if (calculateNodeClipPrecise_iteration % 50000 == 0) {
		// 	std::ofstream dbg("calculateSlice_debug.log", std::ios::app);
		// 	dbg << "calculateNodeClipPrecise call #" << calculateNodeClipPrecise_iteration
		// 		<< " | node i=" << i
		// 		<< " | seqOffset=" << seqOffset
		// 		<< std::endl;
		// 	dbg.close();
...
// #endif
	static __attribute__((noinline)) NodeCalculationResult calculateNodeInner(const Params& params, size_t i, typename NodeSlice<LengthType, ScoreType, Word, true>::NodeSliceMapItem& slice, const EqVector& EqV, typename NodeSlice<LengthType, ScoreType, Word, true>::NodeSliceMapItem previousSlice, const std::vector<EdgeWithPriority>& incoming, ExistenceCheckFunction bandCheck, NodeChunkType nodeChunks, const WordSlice extraSlice, WordsliceCallback callback, ScoreType seqOffset)
	{
		...
		// // --- Debug logging block ---
		// static int calculateNodeInner_iteration = 0;
		// calculateNodeInner_iteration++;
		// if (calculateNodeInner_iteration % 50000 == 0) {
		// 	std::ofstream dbg("calculateSlice_debug.log", std::ios::app);
		// 	dbg << "calculateNodeInner call #" << calculateNodeInner_iteration
		// 		<< " | node i=" << i
		// 		<< " | seqOffset=" << seqOffset
